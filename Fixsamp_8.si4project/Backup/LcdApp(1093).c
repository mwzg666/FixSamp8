...
{
    BYTE i = 0;
    LcdCmd(LCD_CMD_WRITE, REG_SP_VALVE1, (BYTE *)SysParam.Valve, 16);
}
    ...
    DebugInt(addr);
     //printf("RemCtlFlag = %d\r\n",SysParam.RemCtlFlag);
        switch (addr)
        {
            ...
            // 参数设置界面
            case REG_SP_FLOW1:  printf("F1\r\n");   SysParam.SampFlow[0] = ((float)PopWord())/10;  break; //0x4000
            case REG_SP_FLOW2:  printf("F2\r\n");   SysParam.SampFlow[1] = ((float)PopWord())/10;break; //   0x4001
            case REG_SP_FLOW3:  printf("F3\r\n");   SysParam.SampFlow[2] = ((float)PopWord())/10;break; //   0x4002
            case REG_SP_FLOW4:  printf("F4\r\n");   SysParam.SampFlow[3] = ((float)PopWord())/10;break; //   0x4003
            case REG_SP_FLOW5:  printf("F5\r\n");   SysParam.SampFlow[4] = ((float)PopWord())/10;break; //   0x4004
            case REG_SP_FLOW6:  printf("F6\r\n");   SysParam.SampFlow[5] = ((float)PopWord())/10;break; //   0x4005
            case REG_SP_FLOW7:  printf("F7\r\n");   SysParam.SampFlow[6] = ((float)PopWord())/10;break; //   0x4006
            case REG_SP_FLOW8:  printf("F8\r\n");   SysParam.SampFlow[7] = ((float)PopWord())/10;break; //   0x4007
            
            case REG_SP_ADDR:   printf("ADDR\r\n"); SysParam.Address = (BYTE)PopWord();             break; //   0x4008
            case REG_SP_TIME:   printf("TIME\r\n"); SysParam.SampTime = PopWord();                  break; //   0x4009
            case REG_SP_VOL:    printf("VOL\r\n");  SysParam.SampVol = ((float)PopWord())/10;       break; //   0x400A
            case REG_SP_LIGHT:  printf("LIGHT\r\n");SetBkLight(true);                               break; //   0x400B
            case REG_SP_THRES:  printf("THRES\r\n");SysParam.AlarmThres = (BYTE)PopWord();          break; //   0x400C
            case REG_SP_MODE:   printf("MODE\r\n"); SetSampMode();                                  break; //   0x400D
            case REG_CH_ONOFF:  printf("CHON\r\n"); SysParam.Enable = (BYTE)PopWord();              break;
            case REG_SP_RET:    printf("RET\r\n");  ModeHint(); UpdataUI(); SaveParam();            break;   // 点击参数界面返回按钮
            ...
            // 修改时间
            case REG_ADJ_TIME:   printf("ADJ_TIME\r\n"); GetInputTime();   break;    // 时间修改完成，点击了返回按钮
            case REG_DATE_TIME:  printf("DATA_TIME\r\n"); SetLcdTime();    break;      // 修改屏幕RTC      
            case LCD_REG_RTC:    printf("REG_RTC\r\n"); SetCurTime();      break;      // 获取屏幕时间返回
